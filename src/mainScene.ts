type ChickenType = 'chicken' | 'turkey';
type ChickenSprite = Phaser.Physics.Arcade.Image;

interface Bush {
  x: number;
  y: number;
  radius: number;
  slots: (ChickenSprite | null)[];
}
...
function arcadeBody(sprite: ChickenSprite): Phaser.Physics.Arcade.Body {
...
class MainScene extends Phaser.Scene {
  ...
  bushes: Bush[] = [];
  grassGroup!: Phaser.GameObjects.Group;
  grassRemaining = 0;
  ...
  playWinSound() {
    ...
    [523.25, 659.25, 783.99, 1046.5].forEach((f, i) =>
      playTone(ctx, f, 0.22, 'triangle', 0.18, i * 0.15));
  }

  playGameOverSound() {
    if (!this.audioCtx) return;
    const ctx = this.audioCtx;
    [400, 320, 260, 180].forEach((f, i) =>
      playTone(ctx, f, 0.3, 'sawtooth', 0.14, i * 0.18));
  }

  playNetUnlockSound() {
    if (!this.audioCtx) return;
    const ctx = this.audioCtx;
    [700, 1000, 1300].forEach((f, i) =>
      playTone(ctx, f, 0.15, 'sine', 0.16, i * 0.08));
  }
  ...
  updateScoreText(timeRemainingMs?: number) {
  ...
  spawnChicken(x?: number, y?: number, type?: ChickenType): ChickenSprite | null {
    type = type || 'chicken';
    if (this.chickens && this.chickens.length >= MAX_CHICKENS) return null;
    ...
    head.setDepth(5);
    if (type === 'turkey') head.setScale(0.82);
  ...
  herdChicken(chicken: ChickenSprite) {
    if (chicken.getData('herded') || chicken.getData('delivered')) return;
  ...
  getSpeedMult(chicken: ChickenSprite): number {
    let mult = 1;
    if (chicken.getData('type') === 'turkey') mult *= TURKEY_SPEED_MULT;
    if (chicken.getData('brooding')) mult *= HEN_SPEED_MULT;
    return mult;
  }

  getEatMult(chicken: ChickenSprite): number {
    let mult = 1;
    if (chicken.getData('type') === 'turkey') mult *= TURKEY_EAT_MULT;
    if (chicken.getData('brooding')) mult *= HEN_EAT_MULT;
    return mult;
  }

  pickNewWanderVelocity(chicken: ChickenSprite, time: number) {
  ...
  updateHiding(chicken: ChickenSprite): boolean {
    const existing: BushSlot | undefined = chicken.getData('bushSlot');
    if (existing) return true;

    // Індики та квочки не ховаються в кущах - лише звичайні курчата
    if (chicken.getData('type') === 'turkey' || chicken.getData('brooding')) return false;
    ...
    for (const bush of this.bushes) {
      ...
      if (dist < bush.radius + HIDE_RADIUS) {
        ...
        if (freeIdx !== -1) {
          ...
          chicken.setData('bushSlot', { bush, idx: freeIdx } as BushSlot);
  ...
  releaseBushSlot(chicken: ChickenSprite) {
    const slot: BushSlot | undefined = chicken.getData('bushSlot');
    if (slot) {
  ...
  useNet(pointer: Phaser.Input.Pointer, currentlyOver: Phaser.GameObjects.GameObject[]) {
    if (!this.netActive || this.gameEnded) return;
    ...
    const directChickens = new Set<ChickenSprite>();
    (currentlyOver || []).forEach((obj: any) => {
      const owner = obj.getData && obj.getData('owner');
      if (owner) directChickens.add(owner);
      else if (this.chickens.includes(obj)) directChickens.add(obj);
    });

    const targets = this.chickens.filter(c =>
      !c.getData('delivered') && !c.getData('herded') &&
  ...
  showNetEffect(x: number, y: number) {
    const g = this.add.graphics();
  ...
  eatNearbyGrass(chicken: ChickenSprite, multiplier: number) {
    if (this.grassRemaining <= 0) return;
    let toEat = multiplier || 1;
    const tufts = this.grassGroup.getChildren() as Phaser.GameObjects.Image[];
    for (let i = 0; i < tufts.length && toEat > 0; i++) {
      const tuft = tufts[i];
      if (!tuft.active) continue;
      const dist = Phaser.Math.Distance.Between(chicken.x, chicken.y, tuft.x, tuft.y);
      if (dist < EAT_RADIUS) {
        tuft.destroy();
        this.grassRemaining--;
        toEat--;
        if (this.grassRemaining <= 0) break;
      }
    }
  }
  ...
  update(time: number, delta: number) {
    ...
    for (const chicken of this.chickens) {
      if (chicken.getData('delivered')) continue;

      const type: ChickenType = chicken.getData('type');
      const herded = chicken.getData('herded');

      const headSprite: Phaser.GameObjects.Image | undefined = chicken.getData('head');
      ...
      if (herded) {
        ...
        const targetX = Phaser.Math.Clamp(
          PEN_CENTER.x + chicken.getData('penOffsetX'),
          PEN_ZONE.x + 15, PEN_ZONE.x + PEN_ZONE.width - 15
        );
        const targetY = Phaser.Math.Clamp(
          PEN_CENTER.y + chicken.getData('penOffsetY'),
          PEN_ZONE.y + 15, PEN_ZONE.y + PEN_ZONE.height - 15
        );

        let herdSpeed = HERD_SPEED;
        if (type === 'turkey') herdSpeed *= TURKEY_SPEED_MULT;
        if (chicken.getData('wasHen')) herdSpeed *= HEN_SPEED_MULT;
        ...
        if (dist < 6 || Phaser.Geom.Rectangle.Contains(
          new Phaser.Geom.Rectangle(PEN_ZONE.x + 10, PEN_ZONE.y + 10, PEN_ZONE.width - 20, PEN_ZONE.height - 20),
          chicken.x, chicken.y
        )) {
          arcadeBody(chicken).setVelocity(0, 0);
          ...
          let points = 1;
          if (type === 'turkey') points = TURKEY_POINTS;
          else if (chicken.getData('wasHen')) points = 2;
        ...
        if (hidden) {
          ...
          arcadeBody(chicken).setVelocity(0, 0);
          const slot: BushSlot = chicken.getData('bushSlot');
          ...
          if (chicken.getData('brooding')) headSprite!.setTint(0xd2833f);
          else headSprite!.clearTint();
      ...
      if (speed > 1) {
        ...
        if (chicken.texture.key !== frame) chicken.setTexture(frame);
